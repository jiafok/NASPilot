...
const getFields = (t: (key: string, opts?: any) => string): PluginField[] => [
  { key: 'rss_urls', label: 'RSS URLs', type: 'textarea', placeholder: 'https://example.com/rss.xml', required: true, help: t('plugins.ptRss.rssUrlsHelp') },
  { key: 'qbittorrent', label: 'qBittorrent', type: 'object', fields: [
    { key: 'url', label: 'URL', type: 'string', placeholder: 'http://10.0.0.5:8080', required: true, help: t('plugins.ptRss.qbtUrlHelp') },
    { key: 'username', label: 'Username', type: 'string', default: 'admin' },
    { key: 'password', label: 'Password', type: 'password' },
  ]},
  { key: 'download_dir', label: 'Download Directory', type: 'string', placeholder: '/downloads/pt', help: t('plugins.ptRss.downloadDirHelp') },
  { key: 'min_free_gb', label: 'Min Free Space (GB)', type: 'number', default: 50, help: t('plugins.ptRss.minFreeGbHelp') },
  { key: 'max_active_downloads', label: 'Max Active Downloads', type: 'number', default: 15 },
  { key: 'free_check', label: 'Free Check', type: 'boolean', default: false, help: t('plugins.ptRss.freeCheckHelp') },
  { key: 'cleanup', label: 'Cleanup', type: 'object', fields: [
    { key: 'seed_days', label: 'Seed Days', type: 'number', default: 2, help: t('plugins.ptRss.seedDaysHelp') },
    { key: 'stuck_download_days', label: 'Stuck Download Days', type: 'number', default: 3, help: t('plugins.ptRss.stuckDownloadHelp') },
    { key: 'emergency_threshold_gb', label: 'Emergency Threshold (GB)', type: 'number', default: 20, help: t('plugins.ptRss.emergencyThresholdHelp') },
    { key: 'emergency_target_gb', label: 'Emergency Target (GB)', type: 'number', default: 30, help: t('plugins.ptRss.emergencyTargetHelp') },
  ]},
  { key: 'free_ttl_hours', label: 'Free TTL (hours)', type: 'number', default: 48, help: t('plugins.ptRss.freeTtlHelp') },
  { key: 'rss_missing_threshold', label: 'RSS Missing Threshold', type: 'number', default: 2, help: t('plugins.ptRss.rssMissingThresholdHelp') },
  { key: 'enable_rss_eviction', label: 'Enable RSS Eviction', type: 'boolean', default: true },
  { key: 'gc', label: 'Processed GC', type: 'object', fields: [
    { key: 'evicted_days', label: 'Evicted Retention (days)', type: 'number', default: 15, help: t('plugins.ptRss.evictedRetentionHelp') },
    { key: 'expired_days', label: 'Expired Free Retention (days)', type: 'number', default: 5, help: t('plugins.ptRss.expiredFreeRetentionHelp') },
  ]},
];
...
export default function PT_RSS() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (err?.code === 'ECONNABORTED' || err?.message?.includes('timeout')) {
        setRunResult({ status: 'error', error: t('plugins.ptRss.timeoutError') });
      } else {
      ...
      <div>
        ...
        {!r.added_messages?.length && !r.failed_messages?.length && (
          <div style={{ marginTop: 12, padding: 16, textAlign: 'center', color: '#888' }}>
            <InfoCircleOutlined /> {t('plugins.ptRss.noRecords')}
          </div>
        )}
      </div>
    );
          ...
          <Tooltip title={t('common.delete')}>
            <Button type="link" danger size="small" icon={<DeleteOutlined />}
              onClick={() => Modal.confirm({
                title: t('plugins.ptRss.confirmDelete', { tid: record.tid }),
  ...
  const logPanel = (
    <div style={{ marginBottom: 16 }}>
      <LogViewer source="pt_rss" maxHeight={400} placeholder={t('plugins.ptRss.logPlaceholder')} collapsible defaultOpen={false} label={t('plugins.ptRss.runningLog')} />
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <Modal
        title={editTid ? t('plugins.ptRss.editRecord', { tid: editTid }) : ''}
        open={editVisible}
        onOk={handleEditSave}
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          <div>
            ...
            <Input value={editForm.title || ''} onChange={(e) => setEditForm({ ...editForm, title: e.target.value })} placeholder={t('plugins.ptRss.torrTitle')} />
          </div>
          <div>
            ...
            <Select style={{ width: '100%' }} value={editForm.status || 'pending_free'}
              ...
              options={[
                { label: 'pending_free (' + t('plugins.ptRss.status.pending_free') + ')', value: 'pending_free' },
                { label: 'added (' + t('plugins.ptRss.status.added') + ')', value: 'added' },
                { label: 'completed (' + t('plugins.ptRss.status.completed') + ')', value: 'completed' },
                { label: 'evicted (' + t('plugins.ptRss.status.evicted') + ')', value: 'evicted' },
                { label: 'expired_free (' + t('plugins.ptRss.status.expired_free') + ')', value: 'expired_free' },
              ]}
            />
          </div>
          <div>
            <Typography.Text strong>{t('plugins.ptRss.evictionReason')}</Typography.Text>
            <Input value={editForm.evicted_reason || ''} onChange={(e) => setEditForm({ ...editForm, evicted_reason: e.target.value })} placeholder={t('common.optional')} />
          </div>
          <div>
            <Typography.Text strong>{t('plugins.ptRss.evictionTime')}</Typography.Text>
